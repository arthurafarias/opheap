...
#include "opheap/detail/ordered_roots.hpp"
#include "opheap/detail/snapshot_format.hpp"
...
inline snapshot_image snapshot_store::load() const {
...
inline snapshot_image snapshot_store::save(const snapshot_image& image,
...
inline void snapshot_store::read(const loc& where, std::uint64_t offset, std::span<std::byte> out) const {
...
inline std::vector<std::byte> snapshot_store::load(const loc& where) const {
