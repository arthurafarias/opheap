...
// command dispatch live in the header-only opheap::cli library under
// libraries/libopheap-cli; this file is only the process front end that turns
// argv/stdin/stdout into calls against that library.

int main(int argc, char** argv) {
    std::vector<std::string_view> arguments(argv + 1, argv + argc);
    return opheap::cli::execute(arguments, std::cin, std::cout, std::cerr);
}
